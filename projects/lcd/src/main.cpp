...
LCDWIKI_SPI my_lcd(MODEL, CS, CD, MISO, MOSI, RST, SCK,
                   LED); //model,cs,dc,miso,mosi,reset,sck,led
//if the IC model is not known and the modules is readable,you can use this constructed function
//LCDWIKI_SPI my_lcd(240,320,CS,CD,MISO,MOSI,RST,SCK,LED); //width,height,cs,dc,miso,mosi,reset,sck,led

void
show_string(const char *str, int16_t x, int16_t y, uint8_t csize, uint16_t fc,
            uint16_t bc, boolean mode) {
...
unsigned long show_text(void) {
    ...
    show_string("* Universal Color TFT Display Library *", CENTER, 3, 1, 0x07E0,
                0, 1);

    my_lcd.Set_Draw_color(128, 128, 128);
    my_lcd.Fill_Rectangle(0, my_lcd.Get_Display_Height() - 15,
                          my_lcd.Get_Display_Width() - 1,
                          my_lcd.Get_Display_Height() - 1);
    show_string("<http://www.lcdwiki.com>", CENTER,
                my_lcd.Get_Display_Height() - 11, 1, 0xFFFF, 0, 1);

    my_lcd.Set_Draw_color(255, 0, 0);
    my_lcd.Draw_Rectangle(0, 15, my_lcd.Get_Display_Width() - 1,
                          my_lcd.Get_Display_Height() - 16);
...
unsigned long show_triangle_function(void) {
    ...
    my_lcd.Draw_Fast_VLine(my_lcd.Get_Display_Width() / 2 - 1, 16,
                           my_lcd.Get_Display_Height() - 32);
    my_lcd.Draw_Fast_HLine(1, my_lcd.Get_Display_Height() / 2 - 1,
                           my_lcd.Get_Display_Width() - 2);
    for (i = 1; i <= (my_lcd.Get_Display_Height() - 32) / 2 / 10; i++) {
        my_lcd.Draw_Fast_HLine(my_lcd.Get_Display_Width() / 2 - 1 - 2,
                               my_lcd.Get_Display_Height() / 2 - 1 - i * 10, 5);
        my_lcd.Draw_Fast_HLine(my_lcd.Get_Display_Width() / 2 - 1 - 2,
                               my_lcd.Get_Display_Height() / 2 - 1 + i * 10, 5);
    }
    for (i = 1; i <= (my_lcd.Get_Display_Width() - 2) / 2 / 10; i++) {
        my_lcd.Draw_Fast_VLine(my_lcd.Get_Display_Width() / 2 - 1 - i * 10,
                               my_lcd.Get_Display_Height() / 2 - 1 - 2, 5);
        my_lcd.Draw_Fast_VLine(my_lcd.Get_Display_Width() / 2 - 1 + i * 10,
                               my_lcd.Get_Display_Height() / 2 - 1 - 2, 5);
    }
    ...
    for (i = 1; i < my_lcd.Get_Display_Width() - 2; i++) {
        my_lcd.Draw_Pixel(i, my_lcd.Get_Display_Height() / 2 - 1 +
                             (sin(((i * 1.13) * 3.14) / 180) * 95));
    }
...
unsigned long show_sinewave(void) {
    ...
    for (i = 1; i < ((my_lcd.Get_Display_Width() - 2) * t); i++) {
        ...
        if (i > my_lcd.Get_Display_Width() - 1) {
            if ((x == my_lcd.Get_Display_Width() / 2 - 1) ||
                (buf[x - 1] == my_lcd.Get_Display_Height() / 2 - 1)) {
                my_lcd.Set_Draw_color(0, 0, 255);
            } else {
        ...
        y = my_lcd.Get_Display_Height() / 2 - 1 +
            (sin(((i * k) * 3.14) / 180) * (90 - (i / 100)));
...
unsigned long show_fill_rectangle(void) {
    ...
    for (i = 0; i < 5; i++) {
        ...
        my_lcd.Fill_Rectangle(x_spec + i * side_len - 1,
                              y_spec + i * side_len - 1,
                              x_spec + (i + 1) * side_len - 1,
                              y_spec + (i + 1) * side_len - 1);
        my_lcd.Fill_Rectangle(x_spec + i * side_len - 1,
                              y_spec + (5 - i) * side_len - 1,
                              x_spec + (i + 1) * side_len - 1,
                              y_spec + (4 - i) * side_len - 1);
    }
    return micros() - time_start;
}
...
unsigned long show_fill_round_rectangle(void) {
    ...
    for (i = 0; i < 5; i++) {
        ...
        my_lcd.Fill_Round_Rectangle(x_spec + i * side_len - 1,
                                    y_spec + i * side_len - 1,
                                    x_spec + (i + 1) * side_len - 1,
                                    y_spec + (i + 1) * side_len - 1, 10);
        my_lcd.Fill_Round_Rectangle(x_spec + i * side_len - 1,
                                    y_spec + (5 - i) * side_len - 1,
                                    x_spec + (i + 1) * side_len - 1,
                                    y_spec + (4 - i) * side_len - 1, 10);
    }
    return micros() - time_start;
}
...
unsigned long show_fill_circle(void) {
    ...
    for (i = 0; i < 5; i++) {
        ...
        my_lcd.Fill_Circle(x_spec + r_len + i * r_len * 2 - 1,
                           y_spec + r_len + i * r_len * 2 - 1, r_len);
        my_lcd.Fill_Circle(x_spec + r_len + i * r_len * 2 - 1,
                           y_spec + (5 - i) * r_len * 2 - r_len - 1, r_len);
    }
    return micros() - time_start;
}
...
unsigned long show_fill_triangle(void) {
    ...
    for (i = 0; i < 5; i++) {
        ...
        my_lcd.Fill_Triangle(x_spec + i * side_len - 1,
                             y_spec + (i + 1) * h_len - 1,
                             x_spec + side_len / 2 + i * side_len - 1,
                             y_spec + i * h_len - 1,
                             x_spec + (i + 1) * side_len - 1,
                             y_spec + (i + 1) * h_len - 1);
        my_lcd.Fill_Triangle(x_spec + i * side_len - 1,
...
unsigned long show_grid_lines(void) {
    ...
    for (i = my_lcd.Get_Display_Height() - 16; i > 16; i -= 5) {
        my_lcd.Draw_Line(1, i,
                         (my_lcd.Get_Display_Height() - 17) * k + 10 - (i * k),
                         16);
    }
    my_lcd.Set_Draw_color(0, 255, 255);
    for (int i = 15; i < my_lcd.Get_Display_Height() - 17; i += 5) {
        my_lcd.Draw_Line(my_lcd.Get_Display_Width() - 2, i,
                         (my_lcd.Get_Display_Height() - 17) * k + 10 - (i * k),
                         my_lcd.Get_Display_Height() - 17);
    }
    return micros() - time_start;
}
...
unsigned long show_random_pixels(void) {
    ...
    for (i = 0; i < 5000; i++) {
        ...
        my_lcd.Draw_Pixel(2 + random(my_lcd.Get_Display_Width() - 4),
                          17 + random(my_lcd.Get_Display_Height() - 34));
    }
    return micros() - time_start;
}
...
unsigned long show_random_lines(void) {
    ...
    for (i = 0; i < 300; i++) {
        my_lcd.Set_Draw_color(random(255), random(255), random(255));
        my_lcd.Draw_Line(2 + random(my_lcd.Get_Display_Width() - 4),
                         17 + random(my_lcd.Get_Display_Height() - 34),
                         2 + random(my_lcd.Get_Display_Width() - 4),
                         17 + random(my_lcd.Get_Display_Height() - 34));
    }
    return micros() - time_start;
}
...
unsigned long show_random_rectangles(void) {
    ...
    for (i = 0; i < 150; i++) {
        my_lcd.Set_Draw_color(random(255), random(255), random(255));
        my_lcd.Draw_Rectangle(2 + random(my_lcd.Get_Display_Width() - 4),
                              17 + random(my_lcd.Get_Display_Height() - 34),
                              2 + random(my_lcd.Get_Display_Width() - 4),
                              17 + random(my_lcd.Get_Display_Height() - 34));
    }
...
unsigned long show_random_round_rectangles(void) {
    ...
    for (i = 0; i < 150; i++) {
        my_lcd.Set_Draw_color(random(255), random(255), random(255));
        my_lcd.Draw_Round_Rectangle(2 + random(my_lcd.Get_Display_Width() - 4),
                                    17 +
                                    random(my_lcd.Get_Display_Height() - 34),
                                    2 + random(my_lcd.Get_Display_Width() - 4),
                                    17 +
                                    random(my_lcd.Get_Display_Height() - 34),
                                    5);
...
unsigned long show_random_circles(void) {
    ...
    for (i = 0; i < 150; i++) {
        my_lcd.Set_Draw_color(random(255), random(255), random(255));
        my_lcd.Draw_Circle(41 + random(my_lcd.Get_Display_Width() - 82),
                           56 + random(my_lcd.Get_Display_Height() - 112),
                           random(40));
    }
    return micros() - time_start;
}
...
unsigned long show_random_triangles(void) {
    ...
    for (i = 0; i < 150; i++) {
        my_lcd.Set_Draw_color(random(255), random(255), random(255));
        my_lcd.Draw_Triangle(2 + random(my_lcd.Get_Display_Width() - 4),
                             17 + random(my_lcd.Get_Display_Height() - 34),
                             2 + random(my_lcd.Get_Display_Width() - 4),
                             17 + random(my_lcd.Get_Display_Height() - 34),
                             2 + random(my_lcd.Get_Display_Width() - 4),
...
unsigned long show_random_bit_map(void) {
    ...
    for (i = 1; i <= 6; i++) {
        my_lcd.Draw_Bit_Map(
                my_lcd.Get_Display_Width() / 2 - 1 - ((len / 2) * 4 / 3) * i,
                my_lcd.Get_Display_Height() / 2 - 1 - (len / 2) * i, 8, 6, buf,
                i * (len / 6));
        delay(100);
    }
    return micros() - time_start;
...
void clear_screen(void) {
    delay(2000);
    my_lcd.Set_Draw_color(0, 0, 0);
    my_lcd.Fill_Rectangle(1, 16, my_lcd.Get_Display_Width() - 2,
                          my_lcd.Get_Display_Height() - 17);
}
...
unsigned long show_total_time(void) {
    ...
    for (i = 0; i < 15; i++) {
        ...
        my_lcd.Print_String(show_str[i],
                            (my_lcd.Get_Display_Width() - 260) / 2 - 1,
                            (my_lcd.Get_Display_Height() - 150) / 2 + i * 10 -
                            1);
        my_lcd.Set_Text_colour(0, 255, 0);
        my_lcd.Print_Number_Int(buf[i],
                                (my_lcd.Get_Display_Width() - 260) / 2 - 1 +
                                200, (my_lcd.Get_Display_Height() - 150) / 2 +
                                     i * 10 - 1, 0, ' ', 10);
    }
    delay(5000);
    return micros() - time_start;
...
void show_end(unsigned long run_time) {
    ...
    my_lcd.Set_Draw_color(255, 0, 0);
    my_lcd.Fill_Round_Rectangle(my_lcd.Get_Display_Width() / 2 - 1 - 120 + 1,
                                my_lcd.Get_Display_Height() / 2 - 1 - 60 + 1,
                                my_lcd.Get_Display_Width() / 2 - 1 + 120 - 1,
                                my_lcd.Get_Display_Height() / 2 - 1 + 60 - 1,
                                5);
    show_string("Running over!", CENTER,
                my_lcd.Get_Display_Height() / 2 - 1 - 40, 1, 0x07FF, 0, 1);
    show_string("That's ok!", CENTER, my_lcd.Get_Display_Height() / 2 - 1 - 30,
                1, 0x07FF, 0, 1);
    show_string("After a few seconds,", CENTER,
                my_lcd.Get_Display_Height() / 2 - 1 - 20, 1, 0x07FF, 0, 1);
    show_string("it will restart.", CENTER,
                my_lcd.Get_Display_Height() / 2 - 1 - 10, 1, 0x07FF, 0, 1);
    show_string("Please wait ...", CENTER, my_lcd.Get_Display_Height() / 2 - 1,
                1, 0x07FF, 0, 1);
    show_string("Total runtime(us):  ", my_lcd.Get_Display_Width() / 2 - 1 - 90,
                my_lcd.Get_Display_Height() / 2 - 1 + 40, 1, 0xFFE0, 0, 1);
    my_lcd.Set_Text_colour(0, 255, 0);
    my_lcd.Print_Number_Int(run_time, my_lcd.Get_Display_Width() / 2 - 1 + 30,
                            my_lcd.Get_Display_Height() / 2 - 1 + 40, 0, ' ',
                            10);
    delay(10000);
}
